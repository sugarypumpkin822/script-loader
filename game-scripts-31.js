...
local function getOptimalPattern()
    ...
    local memoryUsage = game:GetService("Stats").MemoryUsageMbForTag("Script")
    
    if fps > 60 and memoryUsage < 100 then
        return clickPatterns[1]
    elseif fps > 30 and memoryUsage < 200 then
...
local function calculateCollectibleValue(collectible)
    ...
    local priority = collectible:GetAttribute("Priority") or 1
    
    return {
        value = (value * rarity * priority) / (distance * despawnTime),
        rarity = rarity,
        value = value,
        distance = distance,
...
window.GAME_SCRIPTS_31 = GAME_SCRIPTS_31; 