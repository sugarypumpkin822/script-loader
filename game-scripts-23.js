...
local function calculateCollectibleValue(collectible)
    ...
    local baseValue = collectible:GetAttribute("Value") or 1
    ...
    local despawnTime = collectible:GetAttribute("DespawnTime") or math.huge
    
    return {
        value = (baseValue * rarity) / (distance * timeToCollect * despawnTime),
        rarity = rarity,
        baseValue = baseValue,
        distance = distance,
...
local function optimizeCollectionPath(target)
...
local function collectItems()
    ...
    for _, collectible in ipairs(collectibles) do
        if collectible.stats.distance < 50 then
            optimizeCollectionPath(collectible.item)
            firetouchinterest(player.Character.HumanoidRootPart, collectible.item, 0)
            wait(0.1)
            firetouchinterest(player.Character.HumanoidRootPart, collectible.item, 1)
        end
    end
end
...
window.GAME_SCRIPTS_23 = GAME_SCRIPTS_23; 